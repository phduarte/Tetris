...
namespace Gadz.Tetris.Core.Services {
    ...
    public class GameControllerTests {
        ...
        public void DeveIniciar() {
            ...
            Assert.AreEqual(typeof(TabuleiroEstadoJogando), app.State.GetType());
        ...
        public void DevePausar() {
            app.Start();
            Assert.AreEqual(typeof(TabuleiroEstadoJogando), app.State.GetType(), "Não está iniciando");
            app.Pause();
            Assert.AreEqual(typeof(TabuleiroEstadoPausado), app.State.GetType(), "Não está pausando");
        ...
        public void DeveContinuar() {
            ...
            Assert.AreEqual(typeof(TabuleiroEstadoPausado), app.State.GetType(), "Não está pausando");
            app.Continue();
            Assert.AreEqual(typeof(TabuleiroEstadoJogando), app.State.GetType(), "Não está continuando");
        }
        ...
        public void DeveMoverParaBaixo() {
            app.Start();
            app.MoveDown();
            Assert.AreEqual(1, app.CurrentPiecePosition.Y);
        }
